    ...
    using (SqlConnection conn = new SqlConnection(_connString))
    {
        ...
        using (SqlCommand cmd = new SqlCommand(query, conn))
        {
            ...
            using (SqlDataReader rdr = cmd.ExecuteReader())
            {
                while (rdr.Read())
                {
                    // Get the string value directly from the database row safely
                    ...
                    UsersList.Add(new UserItem {
                        ...
                        // If the string is "Y" or "True", it marks it as true. Otherwise, false.
                        ...
                        // Same rule here for the enabled toggle status
        ...
        private void LoadPostalData()
        {
            using (SqlConnection conn = new SqlConnection(_connString))
            {
                string query = "SELECT id, district, sector, location FROM tbl_D_postal";
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    conn.Open();
                    using (SqlDataReader rdr = cmd.ExecuteReader())
        ...
        public IActionResult OnPostAddUser()
        {
            ...
            using (SqlConnection conn = new SqlConnection(_connString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@dev", NewUser.DeviceId ?? (object)DBNull.Value);
